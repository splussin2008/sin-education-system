    ...
    try {
        ...
        for (const material of selectedMaterials) {
            try {
                ...
                const filePath = path.join(uploadDir, material.answer_pdf_path.replace('/uploads/materials/', ''));

                try {
                    await fs.access(filePath);
                } catch {
                    console.error(`Answer PDF file not found for material ${material.id}:`, filePath);
                    continue;
                }
